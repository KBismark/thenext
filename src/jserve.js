              ...
              render: function () {
                ...
                if (jserve.dev) {
                  for (i = 0; i < dependencyModulePaths.length; i++) {
                    jserve.buildSSRString({
                      pathname: dependencyModulePaths[i].pathname,
                    });
                    inline_dependencies += `${JSON.stringify(
                      dependencyModulePaths[i].external
                    )}:${require(dependencyModulePaths[i]
                      .internal).toString()},`;
                  }
                  jserve.buildSSRString({
                    pathname: jsEntryPath.replace(".", ""),
                  });
                } else {
                  for (i = 0; i < dependencyModulePaths.length; i++) {
                    inline_dependencies += `${JSON.stringify(
                ...
                inline_dependencies += `};},writable:false,configurable:false});!${require(jsEntryPath.replace(
                  "./module/",
                  "./ssr/inline_modules/"
                )).toString()}()}();</script>`;
                return (
                  /*'<i class="brk-d-node" j-type="element"></i>'+*/ JSH.ui.render(
                    renderingPage.ref,
                    renderingPage.args
                  )[symbolIdentifier].value + inline_dependencies
                ); //getStarterScript(jsEntryPath)+
