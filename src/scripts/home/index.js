...
function UserCard() {
  ...
  return (
    <view>
      <div
        class="line-border-round"
        style="border-radius:30px;box-shadow:rgba(0, 0, 0, 0.1) 0px 10px 50px;padding:20px;width:320px"
      >
        <div
          key="backImgCon"
          class="light-back"
          style="height:170px;width:100%;overflow-y:hidden"
        >
          <img key="backImg" style="display:none;width:inherit;" />
        </div>
        <div style="display:flex;justify-content:space-between;">
          <div style="display:flex;">
            <div
              key="profileImgCon"
              class="white-back line-border-roundx2"
              style="width:60px;height:60px;margin-left:10px;margin-top:-24px;border-radius:50%;"
            >
              <img
                key="profileImg"
                style="display:none;width:60px;height:60px;border:1px solid #ffffff;border-radius:50%;"
              />
            </div>
          </div>
          <div>
...
function UserActionsCard() {
  return (
    <view>
      <div
        class="line-border-round"
        style="border-radius:30px;box-shadow:rgba(0, 0, 0, 0.1) 0px 10px 50px;padding:20px;width:320px;margin-top:1rem"
      >
        <>
          {UserAction({
            path1:
...
function UserAction({ path1, path2, surface, width }, attributes, leaveMargin) {
  ...
  return (
    <view>
      <button
        class="back-hover cursor white-back"
        style="border-radius:30px;padding:7px 5px 10px 10px;display:block;border:none;width:100%;align-items:center;"
      >
        <div style="margin-top:10px;display:flex;align-items:center;border-radiu:10px;padding:5px;borde:2px solid #f95603;background-colo:#f95603">
          <svg
            key="svg"
            id="Layer_1"
          ...
          <span
            key="name"
            style="margin-left:3px;font-size:18px;font-weight:bold"
          >
            <>{args.actionname}</>
          </span>
        </div>
...
function NavigationLink() {
  return (
    <view>
      <a
        href=""
        class="deep-color back-hover"
        style="display:flex;font-size:18px;font-weight:bold;padding:15px 15px 15px 20px;text-align:left;border-radius:30px;align-items:center;"
      >
        <>{args.icon()}</>
...
function TopicsLink() {
  ...
  return (
    <view>
      <button
        class="line-border-bottom light-hover-less cursor transparent"
        style="padding:10px;border-radius:4px;width:100%;border:none;text-align:left;border-bottom:1px solid #ebe6e3;display:block;"
      >
        <div
          class="deep-color-less"
          style="font-weight:bold;font-size:18px;margin:5px 0px 10px 0px"
        >
          <>{xss(args.title)}</>
        </div>
        <div style="display:flex;justify-content:space-between;">
          <div style="display:flex;align-items:center;">
            <div
              class="line-border-roundx2 light-back"
              style="width:30px;height:28px;border-radius:50%;"
            >
              <img
                key="image1"
                style="display:none;width:30px;height:28px;border-radius:50%;"
                aria-hidden
              />
            </div>
            <div
              class="line-border-roundx2 light-back"
              style="width:30px;height:28px;border-radius:50%;margin-left:-25px"
            >
              <img
                key="image2"
                style="display:none;width:30px;height:28px;border-radius:50%;"
            ...
            <div
              class="line-border-roundx2 light-back"
              style="width:28px;height:28px;border-radius:50%;margin-left:-25px"
            >
              <img
                key="image3"
                style="display:none;width:28px;height:28px;border-radius:50%;"
              />
            </div>
            <div
              aria-hidden
    ...
    <view>
      <div style="display:flex;justify-content:space-between;">
        ...
        <div>
          <nav
            class="line-border-round"
            style="padding:20px;width:300px;margin-top:1rem;"
          >
            <>
              {NavigationLink({
                title: "Home",
            ...
            <div
              class="line-border-top"
              style="margin-top:20px;padding:5px;font-size:14px;"
            >
              <a href="" class="page-color" style="margin:5px;">
                About
              </a>
              <a href="" class="page-color" style="margin:5px;">
                Privacy
...
function AccountsLink() {
  return (
    <view>
      <div
        class="fake-button line-border-bottom cursor light-hover transparent"
        style="padding:7px 10px;border-radius:4px;width:100%;justify-content:space-between;display:flex;align-items:center;"
      >
        <div style="display:flex;align-items:center;">
          <div>
            <div
              key="profileImgCon"
              class="light-back line-border-roundx2"
          ...
          <div style="margin-left:10px">
            <div
              class="deep-color"
              style="margin-top:10px;margin-left:10px;font-weight:bold;font-size:16px;"
            >
              <>{xss(args.fullname)}</>
            </div>
            ...
            <a
              class="deep-color-less"
              href=""
              style="margin-top:5px;margin-left:10px;font-size:15px;display:block;"
            >
              @<>{xss(args.username)}</>
            </a>
          </div>
...
function AccountSuggestion() {
  return (
    <view>
      <div
        class="transparent"
        style="padding:20px 0px;width:340px;margin-top:1rem;"
      >
        <h2
          class="deep-color"
          style="font-size:24px;margin:5px 10px 20px 10px;font-weight:bold"
        >
          Connect with others
        </h2>

        <>
          {AccountsLink({ fullname: "Bismark Yamoah", username: "KBismark_" })}
        </>
        <>
          {AccountsLink({ fullname: "Abigail Twumasi", username: "abi_twum" })}
        </>
        <>
          {AccountsLink({
        ...
        <a href="" class="page-color" style="padding:15px 10px;font-size:15px;">
          See more accounts
        </a>
      </div>
    </view>
...
function TopicsSuggestionSkeleton() {
  return (
    <view>
      <div
        class="light-back line-border-round"
        style="padding:20px;width:340px;margin-top:1rem;"
      >
        <h2
          class="deep-color"
          style="text-align:center;font-size:24px;margin:5px 10px 20px 10px;font-weight:bold"
        >
          Topics
        </h2>
        <div
          class="line-border-bottom cursor transparent"
          style="padding:7px 10px;border-radius:4px;width:100%;"
        >
          <div style="border-radius:15px;padding:7px 0px;background-color:rgba(0,0,0,0.1);width:80%;margin:15px 10px 15px 0px"></div>
          <div style="border-radius:15px;padding:7px 0px;background-color:rgba(0,0,0,0.1);width:70%;margin:15px 10px 15px 0px"></div>
        </div>
        <div
          class="line-border-bottom cursor transparent"
          style="padding:7px 10px;border-radius:4px;width:100%;"
  ...
  return (
    <view>
      ...
      >
        ...
        <>{TopicsLink({ title: "Web development" })}</>
        <>
          {TopicsLink({
            title: "Computer Science and Programming Languages",
          })}
        </>
        <>{TopicsLink({ title: "Technology and Science" })}</>
