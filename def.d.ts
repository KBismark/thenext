...
declare global {
  namespace JSX {
    type Element = BreakerElement;
    interface IntrinsicElements {
      view: BreakerHTMLWrapperTag;
      // HTML
  ...
  interface Breaker {
    ui: {
      ...
      createApp(
        appName: string,
        ref: componentRef,
        sendArgs: (argumentsTaker: argumentTaker) => IArguments,
        dest: HTMLElement
      ): void;
      renderPage(
        appName: string,
      ...
      createComponent(
        fn: (
          this: ThisObject,
          ...args: any[]
        ) => BreakerElement | BreakerTacElement
      ): ComponentClassFunction;
      ...
      createCallback<T = ThisObject>(
        This: T,
        callback: (this: T, ...args: any[]) => void,
        once?: boolean
      ): (...args: any[]) => void;
      createMethod<T = ThisObject>(
        This: T,
        callback: (This: T, ...args: any[]) => void,
        once?: boolean
      ): (...args: any[]) => void;
      ...
      setClass(
        This: ThisObject,
        elementKey: string,
        classObject: { add?: classnames[]; remove?: classnames[] }
      ): void;
      setAttribute(
        This: ThisObject,
        elementKey: string,
        attrObject: HTMLAttributes | ARIAMixins | GlobalEventHandlers
      ): void;
      ...
      BreakerComponent(...args: any[]): componentRef;
      isDetached(): boolean;
    ...
    loadModule(
      module_src: string,
      handler: { onload: () => void; onerror?: () => void; args: any },
      alternate_server_side_module_src?: string
    ): void;
     ...
     * source path everytime it's loaded. `Breaker.version` can be set once and only
     * before your first `Breaker.export` or `Breaker.global` exports. Works only on browser.
     * Fallback on `alternate_server_side_module_src` argument to `Breaker.import.from()` and `Breaker.loadModule()`
     * or alikes.
     *
     * @example
     * Breaker.version = {
  ...
  interface ThisObject {
    ...
    killOndetach: (destroy?: boolean) => void;
    keepStateOndetach: (keep?: boolean) => void;
    onParentRender: ((this: this, ...args: any[]) => void) | null;
  ...
  interface BreakerElement {
    tag:
      | HTMLElementTagNameTypes
      | HTMLElementDeprecatedTagNameTypes
      | SVGElementTagNameTypes;
    attr:
      | HTMLAttributes
      | ARIAMixins
      | GlobalEventHandlers
      | SpecialStyleString
      | SpecialClassName;
    children: BreakerChildNodes;
  }
  interface BreakerTacElement {
    t:
      | HTMLElementTagNameTypes
      | HTMLElementDeprecatedTagNameTypes
      | SVGElementTagNameTypes;
  ...
  type BreakerChildNodes = [string | BreakerDynamicNode | BreakerElement];
  type BreakerDynamicNode = (
    this: ThisObject,
    ...args: any[]
  ) => null | string | number | renderId | BreakerList;
  type BreakerList = [null | string | number | renderId];
   ...
   */
  interface BreakerHTMLWrapperTag {}
  type HTMLTags =
    | JHTMLAttributes
    | ARIAMixins
    | SpecialStyleString
