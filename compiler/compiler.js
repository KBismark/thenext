...
function buildString(node, isSVGNamespaceElement, depth) {
  ...
  return s1 + nodeString.replace(`${ch ? "," : ""}${child_Rep}`, "") + s2;
}
/**
 *
 * @param {{tag:string,attr:{},children:[]}} node
  ...
  if (selfClosingElements[node.tag]) {
    switch (node.tag) {
      ...
      case "IMG":
        ...
        for (i = 0; i < atrris.length; i++) {
          node.attr[atrris[i]] = String(node.attr[atrris[i]]);
          nodeString = nodeString.replace(
            `${attr_Rep}`,
          ...
          if (typeof compiledFests.children[0] !== "string") {
            if (!compiledFests.children[0].JS) {
              ...
              ssrHtml = ssrHtml
                .replace(
                  fests[i],
                  `function htmlString(args){return (\`${buildPreString(
                    compiledFests.children[0]
                  )}\`)}`
                )
